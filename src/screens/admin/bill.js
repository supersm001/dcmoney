...
import {GetAllUserBillDetails, CreateNewPayment, CancelBill} from '../../services/api/users/userapi'
...
export const AdminViewBill = (props) => {
    ...
    async function getfuncion(){
        if(search) return;
        const res = await GetAllUserBillDetails();
        setDetails([...res]);
    ...
    function ShowEmiDetailsFunction(){
        toggleOverlay();
        props.navigation.navigate('AdminSpecficEmiDetails', {id:paymentdetails[0].id})
    }

    function ShowInvoiceDetailsFunction(){
        toggleOverlay();
        props.navigation.navigate('AdminSpecficInvoiceDetails', [{id:paymentdetails[0].id, type:'invoice'}])
    }
